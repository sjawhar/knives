...
- **Scan:** `$HOME`, depth 3 (`~/a/b/c` is depth 3 and is visited; its children are not), skip directory names starting with `.`, never follow symlinks, stop descending at a directory containing `.jj` or `.git`. A directory whose `.jj/repo` is a directory is a checkout candidate; one whose `.jj/repo` is a file is a workspace and is skipped; a `.git`-only directory is not a candidate. No depth knob, no root list.
