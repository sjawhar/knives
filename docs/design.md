...
knives release cut [NAME]      name a new cut of the composition in hand; refuses orphaned, stale, or silently dropped members unless its explicit opt-out records why; a dotted successor of an unpinned release is refused unless --force-new-name --why; never pushes
...
- **Support both dated and fixed release schemes.**
  - **Dated scheme (default):** an unpinned release is edited and republished in place, retaining its former published head under `keep/release-<name>-<sha8>`. A dotted successor is for a predecessor that agent-c main already pins, or an exceptional `--force-new-name --why` invocation.
