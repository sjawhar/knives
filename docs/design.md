...
knives release [NAME]          plan, cut, edit, republish or reap a release under the configured scheme
knives release cut [NAME]      name a new cut of the composition in hand; refuses orphaned, stale, or silently dropped members unless its explicit opt-out records why; whether a dotted successor is warranted is the owner's call from the consumer's trunk lock, not a knives gate; never pushes
knives release republish       retain the old published head as keep/release-<name>-<sha8>, then push that ref and the replacement release name to the release remote
...
knives release include BRANCH  add a branch (or revision) to the release as one new parent; refuses a new fork point
knives release drop BRANCH     remove a branch's parent from the release; the branch and its bookmark are untouched
knives release advance [BR..] [--from SHA]  move member parents to their branches' tips; refuses a fork point outside the release's existing set
...
After the orphan gate and before the content audit, a cut is refused (exit `3`) when it has the same tree and the same parents as the previous cut on the publish remote: the candidate is built by duplicating the in-hand previous release onto the same parents, so its tree and parents always match the local previous release, and the published copy is the one consumers can pin. The one exception is a dated previous release every consumer pin freezes: the edit verbs refuse it and send the operator to a new dated cut, so a verbatim cut under a name that sorts after it is allowed (exit `0`) and says why — that name is the composition the edits then apply to. The exception is judged from the same pin scan the plan printed, against the release the candidate is compared with; a fixed release branch (no other name to take) stays refused. Ahead of both comparisons, a dated name that does not sort after the newest release — by the same date-then-numeric-suffix order the reap keeps the newest name by — is refused (exit `3`) before a candidate is built: the reap that follows every dated cut keeps only the newest name, so the cut would be created and taken straight back in the same command, reported as success. A member rewritten with the same content is a different parent, so a cut of it is a new composition and lands; a previous cut not yet pushed is not compared.
...
- **Support both dated and fixed release schemes.**
  - **Dated scheme (default):** an unpinned release is edited and republished in place, retaining its former published head under `keep/release-<name>-<sha8>` — the encouraged default, since every avoided name is a consumer pin change, keep-ref and notch chain fewer. A dotted successor is for a predecessor a consumer's `main` already pins; the owner reads that from the consumer's own lock and quotes it in the cut notch. knives reports consumer pin state to inform the call and does not adjudicate it.
  - **Fixed scheme (`release_branch` set):** `knives release` takes no name argument and advances the fixed branch in place via a sideways bookmark move (`jj::set_bookmark_anywhere`). The cut carries the *local* composition in hand under this scheme as under the dated one; the published position is read separately from the publish remote-tracking reference (`{fixed}@release` or `{fixed}@origin`) and reported alongside (`release::previous_position`).
