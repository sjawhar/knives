...
A release is a flat octopus merge of feature and fix branches; its parent set is its membership. `include` adds one parent, `drop` removes one, and `advance` moves a member to its branch tip. Each operation preserves the current release's recorded resolution. The fork-point set is a ratchet: status reports any mixed-base release; `include` and `advance` may retain, but never introduce, a fork point. `release rebase` is the only way to change that set.

One branch is both the release member and the upstream pull request head; there is never a second copy. New branches fork from the release's shared base (`knives start` puts them there, or on fetched upstream trunk when no release exists) and are linear past it. A branch that does not compose into the release means the release is behind: rebase the release, never mint a release-lineage sibling. A member whose own history carries a release merge is reported as `stacked-history`, and the plan stops calling the cut `flat`.
...
Before cutting, the orphan gate verifies that no commits exist reachable only from the previous release lineage or its descendants; if commits would be stranded without a remaining bookmark or upstream trunk reaching them, the cut refuses and lists the exact commit IDs. Passing `--allow-drop` overrides this refusal when dropping those commits is intentional. A cut also refuses any recorded member whose branch has advanced unless `--allow-stale-member --why <reason>` records the exceptional decision.

Before the content audit runs, a cut with the same tree and the same parents as the previous cut on the publish remote is refused: a new name would ship nothing and only ask consumers to re-pin. The one exception is a dated release every consumer pins by revision: the edit verbs refuse it (editing in place would reach nobody) and say to cut a new dated release, so a verbatim cut under a name that sorts after it is allowed and says why, and the edits then apply to the new, unpinned name. A fixed release branch (which has no other name to take) stays refused. Before either comparison, a dated name that does not sort after the newest release is refused outright: the reap after a dated cut keeps only the newest name, so such a cut would be created and taken straight back in the same command. A member rewritten with the same content is a new parent, so a cut of it is a new composition. A previous cut not yet pushed has no consumer to protect and is not compared.
...
An edit follows consumer pin state exactly as a rebase does: when every pin of that release is frozen on a revision, editing it in place would reach nobody, so the edit refuses and says to cut a new dated release; a consumer frozen on an older release is not reached either way and does not block the edit. A release agent-c main has never pinned is mutable: retain its old remote head as `keep/release-<name>-<sha8>`, then use `knives release republish` to move and publish the same release name. A dotted successor whose predecessor no consumer pins is refused unless `--force-new-name --why <reason>` records the exception. An edit also refuses when the upstream trunk cannot be resolved, because it is the trunk that separates the release's base parents from its members.
