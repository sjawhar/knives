...
**Rewrite an unpinned release in place. That is the default, and it is encouraged.** A release name is a coordinate consumers pin, not a changelog; until a consumer's `main` pins it, keep editing the same name — `release include|advance|drop`, gate, `release republish` — and let the name absorb every repair and late member. The prior remote head is retained as `keep/release-<name>-<sha8>`, so nothing is lost. Each avoided name is one fewer consumer pin change, keep-ref, notch chain and review round, and one less stale name for the next reader; one fork went from `release/<date>` to `.5` in a day while its consumer's `main` had pinned only three of them, because the owner read "published" and "an open PR pins it" as consumption. They are not: an open PR relocks to the republished head. Cut a dotted successor only when the consumer's `main` pins the predecessor, and quote the consumer trunk's lock line that proves it in the cut notch (`jj file show -r main@origin <lock> | grep …` → the resolved `rev=<name>#<sha>`); no quotable line, no new name. This is the owner's reading of the consumer's own manifests and locks, not a decision a tool makes — an unreadable consumer is a reason to go read it, not a licence for a new name. Consumers install from their locks, so a republish reaches nobody until a deliberate relock; the failure mode to avoid is a runtime that re-resolves a branch *name* at start (one did, and broke every deployed runner venv when its release moved) — that hop pins by commit, and whoever republishes a lower release re-cuts the consumer above it pin-only in the same sitting (`knives consumers <repo>` lists them).
