...
2. **Compose the intended release.** Use the existing `release advance`, `include`, `drop --why`, `rebase` and `cut` commands as documented in `using-knives`, under the integration barrier. Advance repaired members to verified heads. Include an unreleased PR only when it is intended and ready to ship. Before cutting a new dated name after a published release, read the consumer's tree and state where the previous release is consumed and why the new cut is needed with `knives release cut <name> --consumed-by '<where and why>'`. Knives records your statement; it does not verify it — you do, by reading the consumer's tree. Its missing-statement refusal also prints a partial, root-file-only consumer report for context, never as a decision input. Keep required old refs reachable. Do not generate an identical release just to have a new name.
