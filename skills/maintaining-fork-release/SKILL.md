...
2. **Compose the intended release.** Use the existing `release advance`, `include`, `drop --why`, `rebase` and `cut` commands as documented in `using-knives`, under the integration barrier. Advance repaired members to verified heads. Include an unreleased PR only when it is intended and ready to ship. Read actual consumer pin constraints before choosing an in-place edit or a new cut: a consumer frozen on an older release is still behind but does not block editing the current release, while a tool refusal that every pin of the release being edited is frozen on a revision means an in-place edit would reach nobody and the answer is a new cut, not a parallel lineage or forced ref move. Keep required old refs reachable. Do not generate an identical release just to have a new name.
   ...
   **A drop is decided per commit, not per member.** Every parent that leaves the candidate — a `drop --why`, a member the cut's composition gate names (before `--allow-drop` accepts it), a parent of an old release that a rebuild does not re-include — is swept against `<candidate>`, the composition in hand once that parent is out, before it is cut or published. List every commit the released parent commit (not its branch's current tip) brings past `<base>`, the newest upstream trunk commit the candidate contains (`heads(::<candidate> & ::main@upstream)` with the fork's own trunk name, never a newer upstream head): `jj -R <checkout> --ignore-working-copy log -r '<base>..<parent>' --no-graph`. List against the base, never against the release's other parents: a commit that reached the release through two leaving parents looks carried by each from the other's side and vanishes from both lists. Each commit gets its own row in a `record:` notch on the release, giving every path `jj -R <checkout> --ignore-working-copy diff -r <commit> --name-only` lists a verdict on the commit's change there (the same command with `-- <path>` in place of `--name-only`): `carried` (the candidate commit that brings it in), `on trunk` (the upstream commit in `::<base>` that supplies it) or `dropped` (why the release no longer needs it). A path with no verdict is a loss to restore — as a member of its own, classified like any other — not a drop. `members <candidate> --carries <commit>` settles a row only when it says carried: it measures the commit's whole tree from its common ancestor with the target, so an earlier commit that conflicts or is missing makes the ones after it probe the same way, and this per-path reading is what step 1's "needs investigation" means for a conflicted probe. The member-level answers — `drop`'s note that nothing remaining carries the parent, `--why`, `--allow-drop`, a quiet orphan gate (it asks only whether commits stay reachable, and every parent of the previous release still is) — say nothing about the commits behind a parent, and a member's own content can be genuinely superseded while a commit that reaches the release only through it is not. The case: composing one fork's `release/2026-09-25`, the owner dropped `feat/middleman-aws-role-identity` (4428b3f0), and 5463935b among the parents of an old release the cut rebuilt, as superseded — their aws_sts feature had landed upstream and its file, `middleman/src/middleman/anthropic_wif.py`, had been renamed to `credential_broker.py`. That held for the aws_sts half of b2b04566, the feature's own commit, which reached the previous release through those two routes alone. It changes eight files; the rename accounts for two, and among the other six, beside an STS seam the trunk already had, are `StackConfig.middleman_model_discovery_profile` and middleman's `MIDDLEMAN_MODEL_DISCOVERY_PROFILE` discovery auth, whose upstream port was still an open pull request. The recon had listed b2b04566 by name and ruled on it inside a five-commit group under the feature's heading; the plan written for that cut already had a per-commit rule, and it was weighed for the two tips a reviewer had named, never for the commits behind them. A rule applied only where someone pointed is a patch, not a rule. Step 3 has how the loss was caught and what it would have broken.
