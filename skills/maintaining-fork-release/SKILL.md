...
2. **Compose the intended release.** Use the existing `release advance`, `include`, `drop --why`, `rebase` and `cut` commands as documented in `using-knives`, under the integration barrier. Advance repaired members to verified heads. Include an unreleased PR only when it is intended and ready to ship. Read actual consumer pin constraints before choosing an in-place edit or a new cut: a consumer frozen on an older release is still behind but does not block editing the current release, while a tool refusal that every pin of the release being edited is frozen on a revision means an in-place edit would reach nobody and the answer is a new cut, not a parallel lineage or forced ref move. Keep required old refs reachable. Do not generate an identical release just to have a new name.
   ...
   **A drop is decided per commit, not per member.** Every parent that leaves the candidate — a `drop --why`, a member the cut's composition gate names (before `--allow-drop` accepts it), a parent of an old release that a rebuild does not re-include — is swept against `<candidate>`, the composition in hand once that parent is out, before it is cut or published. List every commit the released parent commit (not its branch's current tip) brings past `<base>`, the upstream trunk the candidate contains (`heads(::<candidate> & ::main@upstream)` with the fork's own trunk name, never a newer upstream head): `jj -R <checkout> --ignore-working-copy log -r '<base>..<parent>' --no-graph`, with that revset itself substituted for `<base>`. On a merge-based upstream it can name several commits; the range takes them as one set, and a single one chosen by hand lets the others' trunk history into the list. List against the base, never against the release's other parents: a commit that reached the release through two leaving parents looks carried by each from the other's side and vanishes from both lists. Each commit gets its own row in a `record:` notch on the release, giving a verdict to each hunk of the commit's change on every path `jj --ignore-working-copy diff -r <commit> --name-only` lists (the same command with `-- <path>` in place of `--name-only` shows the hunks). Run both diffs from the checkout root, so the rows hold repo-relative paths (pass one back from anywhere as `root:"<path>"`): through `-R` from another directory jj prints each path relative to the caller's cwd, `../../../src/<fork>/infra/__main__.py`, a row no other directory can read. The verdicts are `carried` (the candidate commit that brings it in), `on trunk` (the upstream commit in `::<base>` that supplies it) and `dropped` (the decision or evidence that nothing needs it). The unit is the hunk, not the path: a hunk is `carried` or `on trunk` only when the commit it cites supplies every line the hunk adds or removes, and a path is settled only when every hunk of it is. "Superseded" and "landed upstream" are `on trunk`, with the upstream commit cited, never a reason for `dropped`; a hunk no upstream commit supplies is not `dropped` on that ground either, because `dropped` cites the notch or ruling that intends the revert, or a search of each consumer's pinned revision showing nothing uses the change. A hunk with no verdict is a loss to restore, however much of its path is settled — as a member of its own, classified like any other — not a drop. `members <candidate> --carries <commit>` settles a row only when it says carried: it measures the commit's whole tree from its common ancestor with the target, so an earlier commit that conflicts or is missing makes the ones after it probe the same way, and this per-hunk reading is what step 1's "needs investigation" means for a conflicted probe. The member-level answers — `drop`'s note that nothing remaining carries the parent, `--why`, `--allow-drop`, a quiet orphan gate (it asks only whether commits stay reachable, and every parent of the previous release still is) — say nothing about the commits behind a parent, and a member's own content can be genuinely superseded while a commit that reaches the release only through it is not. The case: composing one fork's `release/2026-09-25`, the owner dropped `feat/middleman-aws-role-identity` (4428b3f0), and 5463935b among the parents of an old release the cut rebuilt, as superseded — their aws_sts feature had landed upstream and its file, `middleman/src/middleman/anthropic_wif.py`, had been renamed to `credential_broker.py`. That held for the aws_sts half of b2b04566 and 74587d44, the two commits the restore's record names ("b2b04566+74587d44") as how the previous release carried the capability the cut lost: b2b04566, the feature's own commit, which reached the previous release through those two routes alone, and 74587d44, a child of 5463935b that aligns the AWS role profile behavior and reached it only through 4428b3f0. b2b04566 changes eight files; the rename accounts for two, and among the other six, beside an STS seam the trunk already had, are `StackConfig.middleman_model_discovery_profile` and middleman's `MIDDLEMAN_MODEL_DISCOVERY_PROFILE` discovery auth, whose upstream port was still an open pull request; 74587d44 adds that auth's discovery-profile test and the example config's `middlemanModelDiscoveryProfile` key. The seam and the capability share hunks: four of b2b04566's six add both kinds of line, and `infra/lib/config.py` is three hunks, each adding a `middleman_sts_web_identity_enabled` line and directly after it a `middleman_model_discovery_profile` line. Trunk's 358ef368 supplies all three STS lines and neither trunk nor the candidate has a discovery line, so each hunk, and the path, is a loss; ruled per path, config.py reads `on trunk` on 358ef368, and a restore built from the paths left over still lacks the field whose absence is step 3's TypeError. The recon had listed both commits by name and ruled on them inside a five-commit group under the feature's heading, "drop, superseded on target", citing upstream commits for the feature and none for any hunk of either; the plan written for that cut already had a per-commit rule, and it was weighed for the two tips a reviewer had named, never for the commits behind them. A rule applied only where someone pointed is a patch, not a rule. Step 3 has how the loss was caught and what it would have broken.
