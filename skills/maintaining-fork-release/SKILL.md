...
6. The branch's `placement: verdict:` notch or a statement that none exists. If proposing upstream-bound work yourself, complete `fork-work`'s challenge before dispatching.
...
Reject packets missing the `placement: verdict:` notch or substituting approval/tests for evidence; return them to recon.
...
New entries use `recon:`, `rehome:`, `repair:`, `record:`, `verify:`, `decision:` and `handback:`, each with `--evidence`; the placement verdict is its own shape, `placement: verdict: CONSUMER | FORK | UPSTREAM` followed by the red-team's file, written by `knives start --placement` or by hand with every line the tool reads, `knives notch <branch> -m $'placement: verdict: FORK\nalternative: …\nclass: …\njudge: …'` (a verdict missing its `alternative:`, its `class:` — one of `library-defect | gap-others-need | deployment-preference` — or its `judge:` is refused) — a `placement:` note that continues as prose is an ordinary note the tool does not read as a verdict. Preserve and read older formats. A `handback:` plus released claim ends the worker's active ownership, not the overall task. Accept a thread disposition only with evidence: `-> <commit>`, `declined: <reason>`, or `already answered <reply-url>`. Record outstanding promises explicitly, with the reply URL, rather than counting them as fulfilled. Verify new replies actually published before reporting them as addressed.
...
2. **Compose the intended release.** Use the existing `release advance`, `include`, `drop --why`, `rebase` and `cut` commands as documented in `using-knives`, under the integration barrier. Advance repaired members to verified heads. Include an unreleased PR only when it is intended and ready to ship. Read actual consumer pin constraints before choosing an in-place edit or a new cut: a consumer frozen on an older release is still behind but does not block editing the current release, while a tool refusal that every pin of the release being edited is frozen on a revision means an in-place edit would reach nobody and the answer is a new cut, not a parallel lineage or forced ref move. Keep required old refs reachable. Do not generate an identical release just to have a new name.
   **Include-time placement.** Before including a branch, read its placement note yourself — `knives notch <branch>` shows the `placement: verdict:` entry `knives start --placement` recorded — not the author's summary of it: the note carries the verdict, the non-fork alternative the branch rejected, and its classification, which is exactly the judgment an include ratifies. A member without a placement note, or one whose newest verdict is `CONSUMER`, is not included — the tool refuses both — and the answer is not a workaround: a missing note means the placement red-team (skill `fork-work`) never ran, so run it and record its verdict before deciding; a `CONSUMER` verdict means the branch should not exist, so implement the alternative in the consumer and `finish` the branch instead. The newest verdict decides whatever the branch's age: a `CONSUMER` verdict recorded after a cut refuses that member's re-inclusion too. Only a branch with no verdict at all falls back to grandfathering — members already in the release before the gate (any branch a recorded cut or edit names as a parent) recut and advance unasked — and a first-time name is refused whichever verb admits it: `include`, a named or bare `advance` that finds it stacked on a member's tip, or `advance --from`. A bare commit id is included on your word; the gate is a branch gate.

   **Classify every member before composing it, and write the classification into the release notch.** Each member is exactly one of: `upstream-PR <url>` (an open or merged upstream pull request carries it); `fork-scoped, placement <notch anchor or commit>` (its `placement: verdict:` notch and commit body say why it does not live in the consumer and either which ruling it implements — the head implementing that ruling's words — or that it implements none and is a defect fix carried by red→green evidence); or `neither`. `neither` blocks: the member is dropped (`drop --why`) or returned to its author, not composed. The release owner applies `fork-work`'s ownership test here a second time — *if the fork were replaced by upstream main tomorrow, would anyone but us miss this?* — because the consumer repository is where its deployment's operating policy lives (lifetimes and reaping of jobs, caps, quotas, schedules, alerting), and a fork member is for a defect in the library's own machinery or a capability nothing outside the library can provide. A policy-shaped member is composed on the owner's answer to a question with options, quoted in the notch — a default with a valve, not a prohibition. The case: a fixed creation-time lifetime cap on every job was composed into a release with no classification; it was fork-only, cited a ruling it did not implement, and the owner had it dropped — the same behavior was a scheduled job in the consumer's own cluster. `maintaining-inspect` owns the consumer's record of the incident and the deployment state; read it before claiming that a fork-side drop has reached a consumer.
...
5. **Publish and consume deliberately.** A local cut or in-place release edit is not published; a published ref is not consumed unless the authoritative consumer resolves it. Publish the verified release to the registry's publish remote (`release`, otherwise `origin`), verify the remote ref with `knives pushed`, and complete the consumer pin/build/deployment steps required by the user's goal. Do not deploy when explicitly excluded or bypass credentials/approvals. Verify the final resolved and, where required, running revision and exercise the intended behavior there. Re-run verification after a relevant change. Record `record:` and `verify:` on the release with exact candidate, published and consumed commits plus gate/runtime evidence.
   ...
   **The consumer's pin PR discloses the delta.** A pin-bump PR body states every production behavior change the pin carries, in plain numbers, under `## Production behavior delta`: per member the new release adds or moves, what production does differently after the pin, fork-scoped members marked as such; `none` is written out when nothing changes. Disclosure, so the reviewer sees the policy, not a gate. Supply it from the classification above — the pin's author reads the release members and their notches, not the release title. The pin PR behind the case above listed "lifetime cap" and named neither the number nor that only one deployment carried it; that omission is what the section exists to prevent.
