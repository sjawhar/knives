...
New entries use `recon:`, `rehome:`, `repair:`, `record:`, `verify:`, `decision:` and `handback:`, each with `--evidence`; the placement verdict is its own shape, `placement: verdict: CONSUMER | FORK | UPSTREAM` followed by the red-team's file, written by `knives start --placement` or `knives notch <branch> -m "placement: verdict: …"` — a `placement:` note that continues as prose is an ordinary note the tool does not read as a verdict. Preserve and read older formats. A `handback:` plus released claim ends the worker's active ownership, not the overall task. Accept a thread disposition only with evidence: `-> <commit>`, `declined: <reason>`, or `already answered <reply-url>`. Record outstanding promises explicitly, with the reply URL, rather than counting them as fulfilled. Verify new replies actually published before reporting them as addressed.
