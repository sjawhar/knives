...
> **Where a change lives comes before whether it goes upstream.** A managed fork is a product the consumer repository uses; the consumer is where its own deployment's operating policy lives — lifetimes and reaping of jobs, caps, quotas, schedules, alerting, node sizing, who-may-do-what defaults — and is the default home for anything policy-shaped. A fork member is for a defect in the library's own machinery or a capability nothing outside the library can provide, which is also what makes it upstream-bound. `fork-work` carries the test (*if the fork were replaced by upstream main tomorrow, would anyone but us miss this?* No means the consumer): when the fork genuinely looks like the right home for something policy-shaped, that is itself a missing library capability, not a standing exception — the question to the owner is what belongs upstream and how the fork carries it there meanwhile, never whether the fork can keep it without going upstream. A change that belongs in the consumer has no branch in the fork and no upstream PR. There is one question, not two: this callout asks *fork or consumer*; the callout below asks when a fork member's upstream PR opens, never whether one does — a fork member that never goes upstream is not an option. `maintaining-inspect` owns both.

> **Upstream PRs are a placement judgment, not a permission.** Before opening one, load `maintaining-inspect` (with `fork-work` and `using-knives` for the fork mechanics) and write down whether the fork member is upstream material: a defect any user of the library would hit, fixed with evidence (a reproduction or red→green test), is upstream material; a knob the library already exposes or a change only we want was never fork work at all — that is CONSUMER, no branch — and an unproven fix needs the evidence before it counts, not a standing exemption from going upstream. That answer is recorded in the `placement:` notch and the PR body — nobody is asked and no approval is awaited (Sami, 2026-09-16: "I didn't ask to be in the loop for every upstream interaction"). A fix is CONSUMER (no branch) or UPSTREAM: a fork member that carries the change for the consumer from the moment its release is published, and until upstream ships it (single signed commit on the release's shared base → pushed to the fork remote → `knives notch` → tip + red→green evidence handed to whoever holds the release claim, who includes it in the release in hand (cutting the next dated name first, right then, when every consumer pin of that release is frozen on a revision, as agent-c's `rev = "release/…"` pins are) and publishes; no claim held, you do the same yourself → the consumer pins the published name and deploys). The upstream PR is part of the same work and opens alongside; it gates none of those steps (Sami, 2026-09-25: "The time it takes to start using a fork change is exactly equal to the time it takes to make it. There is no extra overhead"). Once an upstream PR is open, its lifecycle — review rounds, body edits, rebases, the close — is the owning session's own work, never parked on a human.
