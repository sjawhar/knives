...
- **UPSTREAM** — a fork member on the release's shared base, reviewed through the release, that also goes upstream: a defect in the library's own machinery or a capability nothing outside the library can provide (a missing field, an endpoint, or a knob with upstream's default preserved), with a minimal reproduction on unmodified upstream at a cited revision (or, for a feature, the general extension case preserving upstream defaults). The fork member exists only to carry the change until upstream ships it; its upstream PR is part of the same work — opened by the release owner's process, after the verdict is notched, from a signed commit with an Omp-Session trailer.
