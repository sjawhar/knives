...
- Required first-line verdict: `PASS (no repairs)`, `PASS per fix (k/k)`, or `FAIL: fix|body|drift|placement`, followed by evidence per question and any newly discovered finding. A question with no object says `none`.
...
Paste the returned verdict into a `verify:` notch verbatim. A malformed verdict returns for clarification. A FAIL blocks publication: repair code/body or resolve actual drift; `placement` returns to `fork-work`'s challenge before further repair. Refresh the packet and obtain a new review. A verdict for an old tree does not approve a new candidate.
