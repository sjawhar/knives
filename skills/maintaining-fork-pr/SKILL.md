...
Your dispatch supplies the registry repo name, upstream slug, PR number/URL, branch, current status row, relevant captured notches/guidance, the shipped `pr-preflight` skill, the orchestrator's address and a run/repo/PR-qualified scratch path. Ask the orchestrator for genuinely missing coordinates; derive current requirements from the repo and its records, not an invented document. Read `fork-work`, `using-knives` and `using-jj` for the tools' contracts.
...
Obtain the fresh-context verdict: spawn the reviewer yourself from this packet when your harness gives you a subagent tool; otherwise write the packet to the sweep's scratch directory and send its path to the orchestrator, who dispatches the reviewer and returns the verdict verbatim. Either way the owner records the returned verdict verbatim, may spawn or request gate runners the same way, and reconciles their output. Wait up to 30 minutes; an acknowledgement is not a verdict. On timeout, write `decision: reviewer verdict outstanding; repair commits <ids>; bookmark not moved` with evidence and hand back. Obtaining the verdict and resuming verification is the owner's work, not the human's.
