...
With no arguments or subcommand, plans a release: reports member freshness, local branches outside the release, stacked member history, consumer pin state, and a `mixed-base` finding for a release spanning fork points. Planning and edits are local; `release republish` is the one release verb that pushes, and it pushes only the release and its retention ref to the configured release remote.

A release is a flat octopus merge of feature and fix branches, and its parent set is the membership. The upstream base is reached through each member, never carried as a new member. `include`, `drop`, and `advance` preserve a release's recorded resolution while changing its parent set. Fork-point integrity is a ratchet: existing mixed-base releases are reported and can advance a member on an existing fork point, but `include` and `advance` refuse to introduce a new fork point. Only `release rebase` changes the member base set.
...
- **Rebasing a branch onto a newer trunk is sometimes necessary** — a maintainer asks for it, or the branch needs newer upstream. Rebase the whole release first: `knives release rebase` is the only operation that changes the member fork-point set. A subsequent `advance <branch>` may move the rebased member only when that fork point is already in the release; otherwise it refuses and names the rebase. Which fork point a member uses is reported as `mixed-base` when the release has more than one.
...
- Dated scheme (default, when `release_branch` is absent): valid names are exactly `release/YYYY-MM-DD` and `release/YYYY-MM-DD.N`; there is no `.N.M`. **Rewrite in place by default.** A name that no consumer's `main` pins is mutable: `include|advance|drop` it, gate it, and `release republish` under the **same** name (the old remote head is retained as `keep/release-<name>-<sha8>`). This is encouraged, not tolerated — every name you do not mint is a pin change, a keep-ref and a notch chain nobody has to read. Cut a dotted successor only when a consumer's `main` pins the predecessor; that is your reading of the consumer's own lock (quote the resolved `rev=<name>#<sha>` line in the cut notch), not something knives decides — `knives release` reports consumer pin state to inform you, it does not adjudicate. An open PR that pins a name is not consumption: it relocks to the republished head. Hand-composed releases are forbidden.
...
- `knives release cut [NAME] [--allow-drop] [--allow-stale-member --why <reason>]`: audits a candidate cut of the composition in hand. It refuses a member whose recorded commit is no longer its branch tip unless `--allow-stale-member --why` records the exception. Whether a dotted successor is warranted is the owner's call from the consumer's `main` lock (scheme variants above); `cut` does not refuse a name on consumer state. A passing cut creates and names the release locally in one operation.
- The composition gate holds a candidate against the previous cut's ledger event, refusing members it no longer carries unless `--allow-drop` records the intended omission. A recorded commit the repository cannot resolve counts as dropped — unverifiable must not read as carried.
- `knives release reap`: reaps superseded dated release bookmarks locally and never touches a remote.
- `knives release rebase [REF]`: moves the whole composition onto one upstream commit. It exits non-zero after the mutation if any moved member or the rebuilt release merge is conflicted, listing every conflict; recorded resolutions otherwise replay as ordinary rebase semantics.
- `knives release republish`: retain the published remote head as `keep/release-<name>-<sha8>`, move the existing release name to the locally verified replacement, then push exactly those two refs to the configured release remote.
- `knives release include <branch> [--why "..."]`: add a branch (or any revision) to the release in hand as one new parent. It refuses a branch that introduces a fork point outside the release's existing fork-point set; use `release rebase` to change that set.
- `knives release drop <branch> --why "..."`: remove one member parent; the branch and its bookmark are untouched.
- `knives release advance [<branch>...] [--from <old-sha>]`: move member parents to their branch tips without growing the release's fork-point set. A bare advance moves every eligible member; named branches move exactly. A newly introduced fork point is refused with that member, its point, and the existing set named.
- `knives release members [REF] [--verify] [--carries REV] [--census] [--no-github]`: reads direct parents, current holders, advanced tips, and optional content replay evidence.
- `knives release --consumer <DIR>` scans an ad-hoc local consumer alongside recorded consumer slugs. A recorded or passed consumer that cannot be read is a problem, and release edits refuse until it can.
