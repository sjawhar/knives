...
`start` gates branch creation on a placement verdict: a new branch — one that exists nowhere yet — is refused without `--placement <file>`, with the forcing question verbatim: `Starting a new branch in a fork. Are you sure there is no way to get this effect without changing the upstream library, in a way that is not hacky? Consult the placement red-team (skill fork-work) and pass its verdict: knives start <branch> --placement <file>.` The file's first line is `verdict: CONSUMER | FORK | UPSTREAM`, and three more lines are read and required: `alternative:` naming the non-fork mechanism the branch rejected (the branch's whole premise), `class:` with one of `library-defect | gap-others-need | deployment-preference` (the brief's reasoning, spelled as the brief spells it), and `judge:` naming the red-team reviewer the verdict is accountable to; a file missing any of them is refused with the line named, nothing claimed (the `fork-work` skill has the red-team brief that produces all four, and the free text after them); its whole content is recorded on the branch as a ledger note that begins `placement: verdict:` — the only note shape the tool reads as a verdict; a `placement:` note that continues as prose is an ordinary note and is skipped. A `CONSUMER` verdict starts no branch — the effect belongs in the consumer; when the branch already exists (a bookmark in the checkout, ours or on one of our remotes) the verdict is recorded on it first and the refusal says `knives finish <branch>` retires it. An existing branch is continued without one, and `--force` re-claims keep the existing rule. A `--placement` passed on an existing branch records a fresh verdict whether the `start` claims it, resumes a claim already held, or seizes one; the newest wins. Every registered repository is a fork of the `upstream` its entry requires, so the question applies everywhere. A note that begins `placement: verdict:` but that knives cannot read — a token other than the three, or a missing `alternative:`, `class:` or `judge:` line — is an error, not an absence, at every reader (`start`, `release include`/`advance`, `knives gh`): `<branch>'s newest placement note is not a verdict knives can read (…); record a newer one with `knives notch <branch> -m $'placement: verdict: CONSUMER | FORK | UPSTREAM\nalternative: <the non-fork mechanism it rejected>\nclass: library-defect | gap-others-need | deployment-preference\njudge: <who ruled>'``, exit `3`; the remedy shows every line because a notch missing one is itself that error. A `CONSUMER` verdict on an existing branch goes through the same claim path as any `start`: remotes are fetched first, so a branch another clone pushed since the last fetch is found; a claim someone holds is an existing branch whether or not its first bookmark exists yet; and another owner's claim refuses with the holder's name and the `--force --why` remedy exactly as a plain `start` would, the note landing only on a resume, a take, or a seizure.

A claim names its holder by the harness's own session name — `KNIVES_OWNER` from this tool's OpenCode plugin, `CLAUDE_CODE_SESSION_ID`, or `OMP_SESSION_ID`, whichever the shell carries — so two agents under one OS user are two claimants, and a `start` from the second refuses with the holder's name rather than resuming the first's claim. A shell with none of those is anonymous (`<user>/os-user`), and an anonymous claim can be resumed only from inside its own workspace. `status` prints the holder as `<id>/<kind>`.
...
`finish` hands the claim back and removes the workspace: the jj registration is forgotten, the directory removed, and a Git worktree registration git still holds for that path once the directory is gone (git reports it `prunable`) is removed too, so the path is free for the next `start`; `start` clears such a registration at its own path before creating the workspace. Run it as soon as your active work on the branch stops — including when the work now waits on something external, such as an open pull request in review. A claim means "an agent is working here right now", not "this branch matters": holding one after you stop blocks every other agent from picking the branch up, and releasing one loses nothing. The branch, its bookmark, and any open pull request all survive the release, and the work itself is safe because jj snapshots a working copy into a commit, reachable by change id. `--no-cleanup` keeps the directory, which matters only for files jj never tracked, such as build output or an untracked `.env`. `--superseded-by <branch>` records where the work went.
...
- `knives release cut [NAME] [--allow-drop]`: audits a candidate cut of the composition in hand — the previous release's parents carried verbatim (by commit id, so a member whose bookmark has since become divergent is still carried and never read as dropped), nothing joining and nothing advancing — and names it only when the audit passes: each member's net diff, measured from the members' fork point with the upstream trunk, must be present in the cut tree. Divergence the previous release already carried (a recorded conflict resolution) is reported as carried forward, never refused. A failed audit writes nothing at all; a passing one creates and names the release as one operation. Only the first cut, with no composition to carry, starts from every branch. The orphan gate refuses a cut that would strand commits reachable only from the previous lineage; `--allow-drop` overrides it. A dated name that does not sort after the newest release -- that release's own name, or an older date or suffix -- is refused before anything is built: `<repo>: refusing to cut <name>: release/<newest> is the newest release, so a dated cut takes a name that sorts after it (the reap after a cut keeps only the newest name)`, exit `3`.
- After the orphan gate and before the content audit, a cut with the same tree and the same parents as the previous cut on the publish remote is refused: `<repo>: refusing to cut <name>: identical to release/<previous>@<publish remote> (<commit>); nothing to cut`, exit `3`, nothing created. A new name would ship nothing and only ask consumers to re-pin; a branch enters through `include`, members move with `advance`, the base with `rebase`. The one exception: under the dated scheme, when every pin of the previous release is frozen on a revision, those edits refuse it (they would reach nobody) and point here, so a verbatim cut under a name that sorts after it is allowed and says so — `` <repo>: <name> starts identical to release/<previous>@<publish remote> (<commit>); every pin of release/<previous> is frozen, so this new name is the composition to edit — `include`, `advance`, `drop` or `rebase` it before pushing ``, exit `0` — and those edits then apply to the new, unpinned name before it is pushed. A fixed release branch (which has no other name to take) stays refused as identical; the previous cut's own name and an older name were already refused above. A member rewritten with the same content (a `jj describe`, a rebuild) is a different parent, so a cut of it is a new composition and lands. The comparison is against the *published* copy (the candidate is a duplicate of the in-hand previous release, so its tree and parents always match locally); a previous cut not yet pushed has no consumer to protect and is not compared.
- The composition gate: before publishing, the candidate is held against the previous cut's ledger event — the only record of a parent set that survives the release bookmark moving. A recorded member the candidate does not carry (not a parent, not an ancestor of one or of the base, no rewrite of its change — `jj rebase` keeps the change id — reachable from one, and its net diff absent from the candidate tree) refuses the cut and is named, whether it vanished through a hand-rebuilt merge, an out-of-band bookmark move, or a `drop` since the last cut; a member that landed upstream and entered through the base passes without comment. `--allow-drop` states the drop is intended, and the new cut's event records exactly which members were dropped. A recorded commit the repository cannot resolve counts as dropped — unverifiable must not read as carried.
...
- `include` and `advance` additionally gate a NEW member on the placement verdict `knives start --placement` recorded. The newest verdict decides whatever the branch's age: `the placement verdict says this belongs in the consumer` refuses a name whose newest note says `verdict: CONSUMER`, even a member some cut or edit recorded. Only a branch with no verdict at all falls back to grandfathering, on either of two kinds of evidence: the ledger's — a member some cut or edit event already recorded as a parent — or the repository's, consulted only where the ledger has none — a branch the release in hand carries now: its tip at a parent, or grown past a parent that no cut or edit record ever named a branch for and that no bookmark, local or remote, still holds (so a release whose cut predates the ledger's parent records, or has no cut event at all, keeps its members unasked, while a parent some record does name is that record's branch and nothing else's — a new branch forked from a member's released commit after the member grew is a newcomer, refused). In the recordless shape itself — no cut or edit ever wrote a parent record — the tool cannot tell a grown member from a newcomer forked from its vacated released commit, and `advance` admits the newcomer; the ambiguity closes for the repository the moment any cut or edit writes a record. Release recuts, bare and named `advance`, and `drop`+re-`include` of an existing member are untouched; a name neither kind of evidence knows is refused with `<branch> carries no placement verdict; a fork member states the non-fork alternative it rejected (knives start --placement)`, whichever verb admits it: `include`, a named or bare `advance` that finds a never-composed bookmark succeeding a member still at its tip (a new branch stacked on that member), or `advance --from`. An `advance` that would admit several such names reports every one and moves nothing — `nothing advanced; N of M branch(es) would enter <release> for the first time without a placement verdict` — since moving the rest would deliver a composition nobody asked for. The gate is a branch gate, read from the bookmarks at the commit being included, local or `<name>@<remote>`, whatever spelling named it: `include feat/x`, `include feat/x@origin` (a colleague's pushed branch) and `include <sha>` where the sha is `feat/x`'s tip are all `feat/x`, refused or admitted by its verdict, and the cut or edit records the name (remote refs included) so the member is known later; `include <sha>` of a commit no bookmark names is a bare commit no `start` ever named, included on your word — which is one `jj bookmark delete` away from any branch's tip; the exemption is deliberate, and the `notch` remedy is as cheap. The first cut of a repository composes every carried branch, so it gates each of them the same way — `nothing cut; N of M branch(es) would enter the first release without a placement verdict`, every refused name listed, exit `3`, nothing created; there is no release yet for the repository to grandfather by, so only the ledger answers. Record a verdict for a pre-gate branch with `knives notch <branch> -m $'placement: verdict: FORK\nalternative: …\nclass: …\njudge: …'` (every line the tool reads; a notch missing one is refused) or by re-running `start` with `--placement`. `knives gh` applies the same rule to upstream pull requests, and reads the command line in two halves. The argument grammar is gh's own — cobra's command and verb lookup and pflag's flag grammar over gh 2.98.0's `pr create` and `api` flag tables (`src/commands/gh_args.rs`, audited against the committed `gh … --help` fixtures): flags anywhere in the line, before or after the command word, shorthand clusters expanded (`-dHfeat/x` is draft plus head), `--flag=value` and `-Hvalue` attached, `--` ending flags, gh's own aliases normalised (`pr new` is `pr create`), a flag before the verb (`pr -R <up> create`) handed to the verb as cobra hands it, a bare `-` or empty argument skipped as cobra skips it, a switch's `=value` read as a Go bool (`--help=false` is a switch given false — gh runs the command, so the gate reads on; `-h` is help whatever follows it, and a last bare or true `--help` is help: nothing is gated behind help). A dash-argument the table does not define, or a switch given a value that is not a bool, makes the command unreadable and is refused before anything is read: `knives does not know gh's flag <arg> for <command>: if gh accepts it, add it to PR_CREATE_FLAGS|API_FLAGS in src/commands/gh_args.rs; otherwise remove it`, `<arg> gives a switch a value that is not a bool`. Every other reader is a lookup on that one reading. The comparison half does not reproduce gh's, go-gh's or GitHub's normalisation at all — seven review rounds found that each reproduction had a corner wrong. knives compares only a spelling it can read byte for byte, and refuses every other spelling toward a registered upstream with the canonical one (`src/commands/gh_canon.rs`). A repository is `OWNER/REPO` or `HOST/OWNER/REPO`, every segment of `[A-Za-z0-9._-]` and none of them `.`, `..` or ending in `.git`; that is how `-R`/`--repo`, `GH_REPO` and the endpoint's owner and name are read, and two `-R` are refused naming both rather than read last-wins. A two-part spelling is on gh's default host — and only a two-part spelling asks for it: a three-part `-R HOST/OWNER/REPO` or `GH_REPO`, and an `api` endpoint with an absolute URL, name their host and read no configuration file, so the remedies below clear a refusal about gh's files rather than repeat it — read as go-gh's `DefaultHost` reads it: `GH_HOST` when set (read verbatim, and refused when it is not one canonical segment); else the one host gh is configured for when there is exactly one — read from gh's configuration under `GH_CONFIG_DIR`, else `$XDG_CONFIG_HOME/gh`, else `~/.config/gh`: the `hosts:` block inside `config.yml` when that file has a top-level `hosts` key (the pre-multi-account layout gh still reads and keeps ahead of the other file), else `hosts.yml`'s top-level keys. Both files are read whole, by the grammar gh writes them in, or not at all: block mappings of `key:`, `key: scalar` and `key: {}` lines (the empty map gh writes for a token-less user), keys of `[A-Za-z0-9._:-]` behind at most one layer of matching quotes (so `"hosts" :`, `'ghe.example':`, `ghe.example:8443:` are read), separation whitespace of ASCII space and tab only, indentation of spaces only, at any consistent depth, a scalar on its one line (plain, or in one layer of quotes) followed by nothing but a `# comment`, a UTF-8 BOM, blank and comment lines, one document, keys unique within their map, and the one-line document `{}` that `gh auth logout` of the last account leaves in `hosts.yml` (zero hosts). A quoted key is its content verbatim: `"ghe.example ":` carries a space, outside the charset, and refuses the file. The first line in either file outside that grammar — a flow collection, a list item, a complex key (`? hosts`), an anchor or alias, a tag, a block scalar, a second document (`---`), a bare scalar line, a key or value with a character YAML or a quote layer would decode (`"\x68osts":`, a no-break space), a tab indent, a duplicate key, a control byte — and a `hosts` heading or host key carrying a scalar (`hosts: ~`, `ghe.example: x`), makes the default host unknown: refused naming the file and the line — `gh's <file> is not YAML knives reads (line N: "…"), so the default host cannot be told: state the host in the command (-R HOST/OWNER/REPO, --hostname) or set GH_HOST` — never read past, and never read from the other file (a refused `config.yml` is refused whatever `hosts.yml` says). The cost is by design: a hand-edited file YAML and gh read without complaint may be refused here, with the remedy in the refusal, and the file gh writes never is. `hosts: {}` in `config.yml` is the hosts region, empty; a `config.yml` with no `hosts` key yields to `hosts.yml`; a missing file is zero hosts; else `github.com` — with two or more hosts configured gh uses `github.com` whichever they are (measured). The same default a relative REST path takes when no `--hostname` is stated; the precedence is `--hostname`/the URL's host, then `GH_HOST`, then the single configured host, then `github.com`. Refused, each with `knives compares repositories only as OWNER/REPO or HOST/OWNER/REPO; <where> <what you wrote> is not one: state it that way`: any URL form (`https://…`, `git@host:…`, `ssh://…`), a `.git` suffix, a trailing `/`, a percent-escape, an empty value (`-R ""`, `--repo=` — gh would fall back to `GH_REPO` and the checkout; knives will not guess), a fourth part. A canonical spelling is compared to the registry's own remote URL by one rule, `remote_url::same_remote`: the path without `.git`, case-insensitively (GitHub's own owner and repository comparison), and the host by `remote_url::same_host` — a leading `www.` folded off both sides (a registry that spells its upstream on `www.github.com` names what the canonical `-R o/r` names, as gh's own `normalizeHostname` reads it), then equal to the registered host, or any subdomain of it, case-insensitively (`api.github.com`, `foo.github.com`, `www.www.github.com` are all `github.com`). gh folds every `*.github.com` (and `*.<tenant>.ghe.com`, `*.localhost`) to its parent when choosing the token and the endpoint; the suffix rule is a superset of that fold, so no spelling gh would send to a registered upstream compares as another host, at the cost of over-gating a distinct host that is a subdomain of a registered one. A `gh repo set-default` marker is read as gh writes it and nothing wider: `base` names the marked remote's repository; `OWNER/REPO` names that repository on the marked remote's host, which gh takes from the remote whatever the value says, so a value carrying a host is refused (`remote.<name>.gh-resolved is read only as \`base\` or \`OWNER/REPO\``); two `base` markers rank as gh's `Remotes.Sort` ranks them (`upstream` > `github` > `origin` > the rest). The checkout's remotes themselves are the checkout's own configuration and are normalised (`https`, `ssh`, scp forms) as before. Gated: a `pr create` whose repository — `-R`, else `GH_REPO`, else the marker, else the registered fork's upstream remote — is a registered fork's `upstream`. The checkout's remotes are read as git and gh use them: `git remote -v`, so every `url.<base>.insteadOf` rewrite is applied (the raw `remote.<n>.url` would call an aliased upstream another repository), each remote read the way gh's `TranslateRemotes` reads it, by the canonical remote grammar (`remote_url::classify`): `http(s)://[user@]HOST[:PORT]/OWNER/REPO[.git][/]`, `ssh://[user@]HOST[:PORT]/OWNER/REPO[.git][/]` or the scp form `[user@]HOST:OWNER/REPO[.git]`, HOST of `[A-Za-z0-9-]` labels joined by `.`, PORT digits, OWNER and REPO canonical segments, and nowhere a `%` (an escape gh's parser might accept, decode or reject), a `?` or `#` (text gh validates past where knives reads), whitespace, a control byte, an empty host, port or segment, or a `\` — its fetch URL when that is readable, or a local path or `file://` URL (compared only to its own spelling) with no distinct push URL beside it. The push URL is read only for a remote with NO fetch URL at all (`pushurl` with no `url`): gh reads that remote's LAST push line and no other, readable or not, so knives reads exactly that line — readable, a local path, or the remote is unreadable — and never an earlier push URL it happens to read; a blank URL line (`pushurl =` with no value) is no line to gh's remote pattern and none to knives, so the last push line is the last non-blank one, and a remote with no URL at all is skipped as gh skips it. gh filters the checkout's remotes by the hosts it knows before reading a `gh repo set-default` marker or ranking them (measured: a marker on a readable ghe.example remote with only github.com configured is ignored with its remote and the next-ranked remote is resolved), so knives reads a remote as gh-VISIBLE only when its host folds to one knives can certify gh sees: gh's default host (`GH_HOST`, else the one configured host, else `github.com`), every host in gh's configuration files (with `GH_HOST` set the files are not read, as for the default host), and a stated `--hostname`. Marker resolution and the repository's ranking (`upstream`, `github`, `origin`, then the rest) consider only visible remotes whose URL is a forge URL in the grammar: a local path, a host gh is not configured for, or an unreadable URL never ends the ranking (gh drops the remote and reads the next), and a marker on such a remote is refused naming why (`names a remote on <host>, and gh is not configured for that host, so it ignores the marker: unset it … or state the repository (-R OWNER/REPO)`); with gh's files outside the grammar visibility is unknown and the config refusal fires. The visible target is what a token is minted for. The gate trusts none of that ranking: a creation whose repository is read from the checkout — by marker or by rank, whatever either resolved to, even another repository — sweeps EVERY readable remote, visible or not, for registered upstreams, and the branch's verdict must be UPSTREAM in each entry hit (a FORK, CONSUMER or missing verdict in any refuses, naming the entry when there are several; forks whose origins belong to different owners cannot be stated one head and are refused when no head is given). `origin` the registered upstream beside a shadow `upstream` gh cannot use gates; a marker sending gh to another repository while the checkout still names a registered upstream gates; a failed remotes read refuses. The cost is an over-refusal when a registered upstream sits on a remote gh drops or outranks; the remedy states the repository (`-R OWNER/REPO`), which reads no remote. A remote with a fetch URL knives does not read is unreadable whatever its push URLs say — gh reads a repository from a fetch URL carrying a `?query`, a `#fragment` or a decodable `%` escape, and telling that apart from a fetch URL gh also rejects (a one-segment URL, an invalid escape) is mimicry knives does not do; falling to the push URL certified a decoy while gh created on the upstream. Such a remote leaves the repository the checkout addresses unreadable: refused — `remote <name>'s URL (<url>) is outside the grammar knives compares, so the repository this checkout addresses cannot be read: state the repository (-R OWNER/REPO)` — unless `-R`/`GH_REPO` or the endpoint states the repository, when the remotes are not consulted. The registry's own remotes are held to the same grammar at load (`repos.toml`: a forge URL outside it is a configuration error naming the entry and the role; a local path is allowed), so a comparison never has an unreadable side. The remotes are read from the repository's, the user's and the system's configuration — `GIT_CONFIG_*` environment overrides are not read, as for every git read knives makes; and an ssh remote's host is translated as go-gh's ssh translator translates it, `ssh -G <host>` and its `hostname` answer, so an ssh-config alias names the host it stands for; without `ssh` on PATH or on any failure the host stays as written (gh's own fallback), and `ssh.github.com` folds to `github.com`; the REST creation `api … repos/OWNER/REPO/pulls` with method POST (one `-X`/`--method`, compared case-insensitively — two are refused, `states 2 methods … state one`; a request with fields and no method is a POST, as in gh), at the host the request goes to — an absolute URL's own host (gh sends it verbatim and authenticates by it, whatever `--hostname` or `GH_HOST` say; a `--hostname` that does not fold to it is refused, `states two hosts (--hostname X, URL host Y) … state one`), else `--hostname` (one, canonical), else the default host — where `repos/{owner}/{repo}/pulls` and `repos/:owner/:repo/pulls` — exactly those spellings — are the current directory's repository read as `pr create` reads it; a creation endpoint carrying a `?query` string is refused (`carries a query string, which GitHub may read over the body: state the endpoint as repos/OWNER/REPO/pulls and the fields as -f` — whether GitHub reads a query parameter over the body for this POST is not something knives can verify, so nothing is let through on it; a `#fragment` is never sent and is stripped); a creation that carries `--input` (in any spelling, `-` included) is refused whatever fields sit beside it — gh puts every `-f`/`-F` field on the query string and POSTs the file as the body, and GitHub reads `head` from the body, which knives does not read (`state the head as -f head=<fork-owner>:<branch> and the other fields as -f, without --input`); and a POST is refused rather than compared when its path is percent-encoded anywhere (GitHub decodes, knives does not), has an empty segment, names a segment outside the grammar, is an absolute URL other than `https://api.` + the default host + `/<path>` spelled exactly so (lower-case scheme and host), or is `repositories/<id>/pulls` (`cannot be checked against the registry: state the repository as repos/<owner>/<repo>`; no token is minted for any `repositories/<id>/…` call); and a GraphQL `createPullRequest` mutation in the arguments (matched as a token, at a `graphql` endpoint in any letter case: `createPullRequestReview` and the other review mutations pass everywhere), refused inside a registered fork's checkout — colocated jj or plain git — because its repository is a node id knives cannot read, and passed through uninspected outside one. A gated creation is refused unless the head branch's newest verdict is `UPSTREAM` — `<branch> has placement verdict <verdict>, not UPSTREAM; an upstream pull request opens only for a branch the placement red-team ruled UPSTREAM (knives start --placement)`, or the missing-verdict text above. The head is always stated to gh as `<fork-owner>:<branch>`, the one spelling that names the fork's branch: measured against gh 2.98.0, a bare `--head BRANCH` is `headRefName: "BRANCH"` on the base repository — the upstream's own branch, not the fork's — in a fork clone and outside one alike, and with no `--head` at all gh resolves the branch's push target from git configuration (`@{push}`, `branch.<b>.merge` under `push.default=upstream`), which can be a different branch of the fork than the one checked out. So the one `--head`/`-H` stated (a REST creation's one `-f head=`) is read as `OWNER:BRANCH` with `OWNER` the registered entry's `origin` owner, case-insensitively — read through the same identity reader the registry binds by, so an origin knives reads no owner from (a filesystem path, a `file://` URL, a URL with no `owner/repo` path) is refused with its remedy, `state it as https://<host>/OWNER/REPO in repos.toml`, rather than becoming an empty owner a head could match — and the verdict is `BRANCH`'s (a branch of `[A-Za-z0-9._/-]` not beginning with `-`); a bare `BRANCH` is refused with the remedy `state it as <fork-owner>:<branch>`; another owner's `OWNER:BRANCH` is a branch the ledger never ruled on and is refused naming the fork's spelling; an empty `--head=` is refused (`states an empty head`); two heads are refused naming both (`states 2 heads (a, b); gh would open b while a reader expects a: state one`). With no head stated knives reads the branch in hand — the bookmark on `@` in a jj checkout, git's checked-out branch in a plain clone — looks up its verdict, and adds `--head <fork-owner>:<that>` to the command itself, in both kinds of checkout, so gh never resolves a head knives did not read; with neither, the create is refused (`needs a head branch to check its placement verdict`) rather than let through unverified. Toward the fork's own origin or any unregistered repository, in a jj checkout, the bare bookmark still stands in for git's detached HEAD. `-R`/`--repo` on `gh api` is a knives convention, not a gh flag: gh 2.98.0 refuses it (`unknown shorthand flag: 'R' in -R`, exit 1, before any request), and knives reads it — as the shim did — for token routing and for the repository a placeholder endpoint (`repos/{owner}/{repo}/pulls`) names, so with the flag present the outcome is knives' refusal or gh's own; the flag is passed through as written and no creation can ride it. Token routing follows the same reading: a token is minted only for a canonical owner — from the endpoint (`repos/OWNER/…`, `orgs/OWNER`, `users/OWNER`, on the host the request goes to), a canonical `-R`/`GH_REPO`, a marker, or the checkout's remotes — on a host that folds to `github.com` by the same suffix rule (the App token is a github.com credential; a GHE host, `-R ghe.example/o/r` or `--hostname ghe.example`, mints nothing, as gh sends there without `GH_TOKEN`), and never for an empty or non-canonical owner (a `//` or `git@host:/` path, a percent-escape); a URL-form `-R` on a command the gate does not judge routes nothing and says so once on stderr (`knives gh: no token routed: …`), and gh runs on its own auth. Not inspected: a GraphQL document gh reads from a file (`--input <file>` on `graphql`, `-F query=@file`; `-f` expands no `@file`) — only the arguments are read, and the REST creation's `--input` is refused as above; `gh pr create --dry-run` toward an upstream is gated like the creation it rehearses (by design: the same argv minus `--dry-run` creates, and gh's dry run sends no mutation either way); gh aliases (`gh alias set prc 'pr create …'`) and extensions, which expand inside gh, so knives sees `prc` and passes it through: the gate reads gh's own verbs. Pull requests toward `origin` or any unregistered repository pass untouched. With no registry file at all, nothing is registered so nothing is gated, and stderr says so once — `knives: no registry at <path>; nothing to gate` — before the command runs; a registry file that is present but cannot be read is an error (exit 3, gh not run): nothing passes on a ledger the tool cannot read.
- All three edits share a rebase's two refusals, both `Incomplete`: when every pin *of this release* is frozen on a revision, editing it in place would reach nobody, so cut a new dated release instead — a consumer frozen on an older release is not reached either way and does not block the edit, and a release nothing pins is edited freely; a pin is of this release when it names the branch or freezes the exact commit the publish remote holds for it (a lockfile's `?rev=<sha>`, or a `#<sha>` lock fragment, at that commit); and when the upstream trunk cannot be resolved, nothing can separate the release's base parents from its members, so fetch upstream first.
...
- `knives release members <target> --carries <revision>` asks only whether that exact target carries the revision. Its exit status is direct: `0` for any carried verdict, `1` for `NOT carried` or `conflicted`, and `3` if the target could not be resolved or checked. A target's live/superseded role is irrelevant to this explicit query.
...
- `carried-rebased`: the tree does not replay cleanly, but a rewrite of the revision's change (same change id, a different commit — what `jj rebase` or a conflict resolution leaves) is an ancestor of the target; the evidence is that rewrite. The change is carried as its author rebased it, which is a weaker claim than `carried-rewritten`: a divergent change can name different trees.
- `NOT carried`: replaying leaves a clean, non-empty diff and no rewrite of the change is in the target; the target lacks that content.
- `conflicted`: replaying conflicts and no rewrite of the change is in the target; some content may be present or unrelated work touched the same files, so the result requires judgment.
