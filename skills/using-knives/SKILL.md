...
- `include` and `advance` additionally gate a NEW member on the placement verdict `knives start --placement` recorded. The newest verdict decides whatever the branch's age: `the placement verdict says this belongs in the consumer` refuses a name whose newest note says `verdict: CONSUMER`, even a member some cut or edit recorded. Only a branch with no verdict at all falls back to grandfathering — branches that existed before the gate carry no note, so a member some cut or edit event already recorded as a parent keeps working unasked: release recuts, bare and named `advance`, and `drop`+re-`include` of an existing member are untouched — and a name no recorded cut or edit ever carried is refused with `<branch> carries no placement verdict; a fork member states the non-fork alternative it rejected (knives start --placement)`, whichever verb admits it: `include`, a named or bare `advance` that finds a never-composed bookmark succeeding a member by ancestry (a new branch stacked on a member's tip), or `advance --from`. The gate is a branch gate: `include <commit-id>` — a bare commit no bookmark names — is included on your word, since no `start` ever named it. By design the first cut of a repository composes every branch with no gate, there being no `include` to gate it; the verdict applies from the first `include` or `advance` on. Record a verdict for a pre-gate branch with `knives notch <branch> -m "placement: verdict: FORK …"` or by re-running `start` with `--placement`. `knives gh` applies the same reading to upstream pull requests: `gh pr create` (`--head`/`-H`, or the current bookmark) and the REST creation (`api [-X POST|--method POST|-H …] repos/<owner>/<repo>/pulls` with a body — flags may precede the path; `repos/{owner}/{repo}/pulls` is resolved to the current directory's base repository as gh does) whose resolved target is a registered fork's `upstream` are refused unless the head branch's newest verdict is `UPSTREAM` — `<branch> has placement verdict <verdict>, not UPSTREAM; an upstream pull request opens only for a branch the placement red-team ruled UPSTREAM (knives start --placement)`, or the missing-verdict text above. A GraphQL `createPullRequest` mutation in the arguments is refused inside a registered fork (its repository is a node id knives cannot read; `createPullRequestReview` and the other review mutations pass); a GraphQL document gh reads from a file (`--input <file>`, `-f query=@file`) is not inspected — only the arguments are. Pull requests toward `origin` or any unregistered repository pass untouched. When the registry does not load, the gate is skipped and stderr says so once: `knives: placement gate skipped: <error>`; the command runs as before.
