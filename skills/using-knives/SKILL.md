...
- Dated scheme (default, when `release_branch` is absent): valid names are exactly `release/YYYY-MM-DD` and `release/YYYY-MM-DD.N`; there is no `.N.M`. A release agent-c main has never pinned is mutable: edit it in place, retain the old remote head as `keep/release-<name>-<sha8>`, and republish under the **same** name. A dotted successor is allowed only when agent-c main pins its predecessor; `--force-new-name --why` records the exceptional reason. Hand-composed releases are forbidden.
...
- `knives release cut [NAME] [--allow-drop] [--allow-stale-member --why <reason>] [--force-new-name --why <reason>]`: audits a candidate cut of the composition in hand. It refuses a member whose recorded commit is no longer its branch tip unless `--allow-stale-member --why` records the exception. A dotted successor whose predecessor no consumer pins is refused in favor of editing and republishing that predecessor in place; `--force-new-name --why` is the exceptional opt-out. A passing cut creates and names the release locally in one operation.
