...
`start` gates branch creation on a placement verdict: a new branch — one that exists nowhere yet, in a registry entry with an `upstream` remote — is refused without `--placement <file>`, with the forcing question verbatim: `Starting a new branch in a fork. Are you sure there is no way to get this effect without changing the upstream library, in a way that is not hacky? Consult the placement red-team (skill fork-work) and pass its verdict: knives start <branch> --placement <file>.` The file's first line is `verdict: CONSUMER | FORK | UPSTREAM` (the `fork-work` skill has the red-team brief that produces it, and the rest of the format: `alternative:`, `class:`, `judge:`, free text); its whole content is recorded on the branch as a ledger note prefixed `placement: `. A `CONSUMER` verdict starts no branch — the effect belongs in the consumer. An existing branch is continued without one, and `--force` re-claims keep the existing rule. A `--placement` passed on an existing branch records a fresh verdict; the newest wins.
...
- `include` and `advance --from` additionally gate NEW members on the placement verdict `knives start --placement` recorded: a branch no recorded cut or edit ever named as a parent is refused with `<branch> carries no placement verdict; a fork member states the non-fork alternative it rejected (knives start --placement)` when the ledger holds no `placement:` note for it, and with `the placement verdict says this belongs in the consumer` when the newest one says `verdict: CONSUMER`. Grandfathering: branches that existed before the gate carry no note, so a member some cut or edit event already recorded as a parent keeps working unasked — release recuts, bare and named `advance`, and `drop`+re-`include` of an existing member are untouched; only a first-time inclusion is gated. Record a verdict for a pre-gate branch with `knives notch <branch> -m "placement: verdict: FORK …"` or by re-running `start` with `--placement`. `knives gh` applies the same reading to upstream pull requests: `gh pr create` (and the REST `repos/<owner>/<repo>/pulls` creation) whose resolved target is a registered fork's `upstream` is refused unless the head branch's newest verdict is `UPSTREAM` — `<branch> has placement verdict <verdict>, not UPSTREAM; an upstream pull request opens only for a branch the placement red-team ruled UPSTREAM (knives start --placement)`, or the missing-verdict text above. Pull requests toward `origin` or any unregistered repository pass untouched.
