...
- **A raw `jj rebase -b`/`-s`/`-r` on this branch is a different question from the two verbs
  above.** `knives release advance`/`knives release rebase` are release-aware and correctly
  reshape the release-in-hand's own composition; a bare `jj rebase` is not, and this repository
  can hold commits relied on for their exact identity well outside that one release — a
  superseded dated release a consumer or some other pin is still frozen on, or one kept only
  for its conflict-resolution record (`knives release reap`, below) — sitting directly on this
  branch as a parent with no slot in the release *in hand* at all, invisible to bare `knives
  release members` (see its entry below) precisely because that command only inspects the one
  release in hand. Before rebasing a branch by hand, `fork-work`'s bidirectional check — is the
  branch a parent of a release/keep merge (this bullet's hazard), or does it descend from one
  (the `stacked-history` hazard, below)? — is what actually answers whether that is safe; this
  section's facts about `advance`/`rebase` membership alone do not, and they cover only one of
  the two directions.
...
- **A member's history past the trunk carries no merge.** A branch built on a release merge (or with any release merge in its history) carries every parent of that merge; `include`, `advance` and the first `cut` refuse it with the `stacked-history` detail, the plan says so instead of pointing at `include`, and a member that got in before this check reports `stacked-history` in the plan. The trunk is measured at every known position — `main@upstream`, `main@origin`, local `main` — so a merge one of them reaches is the trunk's, not the branch's; when every local view is behind the branch's base the detail says the merges may be upstream's own and `knives sync` fetches them. A bare `jj rebase -b <branch> -d <trunk>` is not the fix once that release merge has ever been pushed anywhere: `-b`'s root computation still includes the merge itself (it is not yet reachable from the new trunk), so it silently rewrites the very ref this detail is warning about — verified against both a retained cut and a still-current, pushed one. The safe move rebases only the branch's own commits: `jj rebase -s '<release-or-keep-ref>..<branch>' -d <trunk>@upstream` (`fork-work`'s bidirectional check names `<release-or-keep-ref>`), which keeps the branch's change ids so `advance` still follows it, and leaves the release merge's commit id untouched. Plain `-b` is only safe here when that merge has never been pushed anywhere.
